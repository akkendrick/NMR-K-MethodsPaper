function [b_boot,k_boot,totalErrorEstimate] = computeSeevers_all(Kall,logK,...
    logT2ML,logPhi,n,m,saveData)
% Compute Seevers Models for all K data
...
k_boot = median_b*(phi.^m).*((seeversT2.^(-1)-interpT2B.^(-1)).^(-1)).^n;
