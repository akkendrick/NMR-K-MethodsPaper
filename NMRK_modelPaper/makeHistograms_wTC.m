% Make nice histograms of all NMR-K models on Wisconsin data
clear
close all
...
%%
% SOE Model
clear nmrName site siteList Dk d T2ML phi z SumEch kk lt lp SumEch_3s SumEch_twm_3s;
% ! SPECIFY PATH IN THIS FILE
computeSOE
...
text(-1.9,40,strcat('Median K_{diff} = ',num2str(round(medianSDRerrorFactor,2))),'FontSize',14)
...
ylabel('Counts')
set(gca,'FontSize',14)
text(-1.9,40,strcat('Median K_{diff} = ',num2str(round(medianSeeverserrorFactor,2))),'FontSize',14)


subplot(3,2,3)
title('TC Model 33 ms cutoff')

edgesLog = [-3 -2 -1.5 -1 -0.8 -0.6 -0.4 -0.2 0 0.2 0.4 0.6 0.8 1 1.5 2 3];
edges = 10.^edgesLog;
...
ylim([0 50])
%ylim([0 30])
text(-1.2,40,strcat('Median K_{diff} = ',num2str(round(medianTCerrorFactor,2))),'FontSize',14)

xticks([-3,-2,-1,0,1,2,3])%in log space
xticklabels({'-10000','-100','-10','0','10','100','10000'})
xlabel('K Difference Factor')
ylabel('Counts')
set(gca,'FontSize',14)
...
ylim([0 50])
%ylim([0 30])
text(-1.9,40,strcat('Median K_{diff} = ',num2str(round(medianKGMerrorFactor,2))),'FontSize',14)
...
box on
grid minor
text(-1.9,40,strcat('Median K_{diff} = ',num2str(round(medianSOEerrorFactor,2))),'FontSize',14)
...
SDR_numBelow = sum(vertcat(SDR_errorSigns{:}) < KfactorCutoff)
SOE_numBelow = sum(vertcat(SOE_errorSigns{:}) < KfactorCutoff)
Seevers_numBelow = sum(vertcat(Seevers_errorSigns{:}) < KfactorCutoff)
KGM_numBelow = sum(vertcat(KGM_errorSigns{:}) < KfactorCutoff)
TC_numBelow = sum(vertcat(TC_errorSigns{:}) < KfactorCutoff)

% KfactorCutoff = 10;
% 
% SDR_numBelow = sum(plottedSDRerrorFactor < KfactorCutoff)
% SOE_numBelow = sum(plottedSOEerrorFactor < KfactorCutoff)
% Seevers_numBelow = sum(plottedSeeverserrorFactor < KfactorCutoff)
% KGM_numBelow = sum(plottedKGMerrorFactor < KfactorCutoff)
% TC_numBelow = sum(plottedTCerrorFactor < KfactorCutoff)
