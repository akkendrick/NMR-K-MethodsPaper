% Plot all K, T2ml, water table profiles with surface relaxivity
...
%     T2dist = T2dist(:,2:end);
    ...
    rhoDepths = StartingDepthm;
    rhoVal = SurfaceRelaxivityms;
    plotWellProfiles_wRho(waterTable(jj),z,T2dist,T2logbins,DPPdat,SDR_K_all, SDR_K_best,...
        plotNum, f1, coreSampleLoc, rhoDepths, rhoVal)
    title(figName)
