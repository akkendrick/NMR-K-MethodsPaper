...
if isempty(m) && isempty(n)
    ...
    for i = 1:length(siteList)
        ...
        [K,z,T2dist,T2logbins,k_boot,k_mcmc,k_direct,bestFitMatrix,...
            b_boot,totalErrorEstimate] = computeProfile_Kfilt(siteName,[],[],figureson,wDirect,saveData);
...
elseif isempty(m) && ~isempty(n)
    for j = 1:length(n)
        ...
        for i = 1:length(siteList)
            siteName = siteList{i};
            [K,z,T2dist,T2logbins,k_boot,k_mcmc,k_direct,bestFitMatrix,...
                b_boot,totalErrorEstimate] = computeProfile_Kfilt(siteName,n(j),[],figureson,wDirect,saveData);
       ...
       for i = 1:length(siteList)
            ...
            if m(j) == 99
                [K,z,k_boot,k_mcmc,k_direct,bestFitMatrix,b_boot,...
                    T2ML{i},phi{i},totalErrorEstimate,finalDepths{i}] = computeProfile_modT2ML(siteName,...
