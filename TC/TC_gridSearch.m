...
siteList = [{'Site1-WellG5'} {'Site1-WellG6'} {'Site2-WellPN1'} {'Site2-WellPN2'} {'Site1-WellG6'}];
...
for j = 1:length(siteList)
    ...
    parfor i = 1:length(cutoff)
        ...
        [K,z,T2dist,T2logbins,kTC_best,bestFitMatrix,totalErrorEstimate,meanErrorFactor,medianErrorFactor] = computeTCperm(baseName,n,m,cutoff(i),figureson);
 
        mTemp(i) = bestFitMatrix(3);
        nTemp(i) = bestFitMatrix(2);
        cTemp(i) = bestFitMatrix(1);
        errorTemp(i) = medianErrorFactor;
    end
...
save('optimalCutoffTableKansas_n2_m0_take2.mat','totalmMatrix','totalnMatrix','totalcMatrix','totalErrorMatrix','cutoff','siteList','n','m')
...
load('optimalCutoffTableKansas_n2_m0_take2.mat')
