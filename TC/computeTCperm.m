function [K,z,T2dist,T2logbins,phi,kTC_best,bestFitMatrix,totalError,meanErrorFactor,medianErrorFactor,indexQuotientLog] = computeTCperm(baseName,n,m,cutoff,figureson)
...
if isempty(n) && isempty(m)
    % m, n can vary
    [c_boot, n_boot, m_boot] = bootstrap_fun([indexQuotientLog, logPhi, logK], Nboot);
else
    % n and m fixed
